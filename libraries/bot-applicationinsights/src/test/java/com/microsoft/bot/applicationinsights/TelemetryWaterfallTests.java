...
public class TelemetryWaterfallTests {
    ...
    public void ensureEndDialogCalled() {
        ...
        Assert.assertEquals("Step1of3", saved_properties.get("WaterfallComplete_4").get("StepName"));
        Assert.assertTrue(saved_properties.get("WaterfallStep_1").containsKey("InstanceId")); // This assert is repeated
    ...
    private class MyWaterfallDialog extends WaterfallDialog {
        ...
        public CompletableFuture<Void> endDialog(TurnContext turnContext, DialogInstance instance, DialogReason reason) {
            endDialogCalled = reason == DialogReason.END_CALLED;
            cancelDialogCalled = reason == DialogReason.CANCEL_CALLED;
