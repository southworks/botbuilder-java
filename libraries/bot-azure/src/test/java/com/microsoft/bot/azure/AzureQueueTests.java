...
public class AzureQueueTests {
    ...
    private static String AZURE_STORAGE_EMULATOR_ENDPOINT = "http://localhost:10001";

    @BeforeClass
    public static void allTestsInit() throws IOException, InterruptedException, DocumentClientException {
        Process p = Runtime.getRuntime().exec
            ("cmd /C \"" + System.getenv("ProgramFiles") + "\\Microsoft SDKs\\Azure\\Storage Emulator\\AzureStorageEmulator.exe\" /GetStatus");
    ...
    public QueueClient containerInit(String name) {
        QueueClient queue = new QueueClientBuilder().endpoint(AZURE_STORAGE_EMULATOR_ENDPOINT).buildClient();
        queue.create();
        queue.clearMessages();
        return queue;
    }
    ...
    public void continueConversationLaterTests() {
        ...
        String messageJson = messages.getMessageText();
