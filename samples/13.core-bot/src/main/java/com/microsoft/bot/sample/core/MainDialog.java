...
public class MainDialog extends ComponentDialog {
    ...
    private CompletableFuture<DialogTurnResult> introStep(WaterfallStepContext stepContext) {
        if (!luisRecognizer.isConfigured()) {
            Activity text = MessageFactory.text("NOTE: LUIS is not configured. To enable all capabilities, add 'LuisAppId', 'LuisAPIKey' and 'LuisAPIHostName' to the appsettings.json file.", null, InputHints.IGNORING_INPUT);
        ...
        return luisRecognizer.recognize(stepContext.getContext()).thenCompose(luisResult -> {
            switch (luisResult.getTopScoringIntent().intent) {
                            ...
                            BookingDetails bookingDetails = new BookingDetails() {
                                {
                                    ...
                                    setDestination(toEntities.get("airport").toString());
                                    setOrigin(fromEntities.get("airport").toString());
                                    setTravelDate(luisRecognizer.getTravelDate(luisResult));
                                }
                            };
                ...
                default:
                    ...
                    String didntUnderstandMessageText = String.format("Sorry, I didn't get that. Please try asking in a different way (intent was %s)", luisResult.getTopScoringIntent().intent);
    ...
    private static CompletableFuture<Void> showWarningForUnsupportedCities(TurnContext turnContext, ObjectNode fromEntities, ObjectNode toEntities) {
        ...
        if (StringUtils.isNotBlank(fromEntities.get("from").toString()) && StringUtils.isBlank(fromEntities.get("airport").toString())) {
            unsupportedCities.add(fromEntities.get("from").toString());
        }

        if (StringUtils.isNotBlank(toEntities.get("to").toString()) && StringUtils.isBlank(toEntities.get("airport").toString())) {
    ...
    private CompletableFuture<DialogTurnResult> finalStep(WaterfallStepContext stepContext) {
        ...
        if (stepContext.getResult() instanceof BookingDetails) {
            ...
            String messageText = String.format("I have you booked to %s from %s on %s", result.getDestination(), result.getOrigin(), travelDateMsg);
            Activity message = MessageFactory.text(messageText, messageText, InputHints.IGNORING_INPUT);
