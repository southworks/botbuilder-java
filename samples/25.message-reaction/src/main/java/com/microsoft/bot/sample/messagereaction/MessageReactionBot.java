...
public class MessageReactionBot extends ActivityHandler {
	private static final ActivityLog log;
	...
	protected CompletableFuture<Void> onReactionsAdded(List<MessageReaction> messageReactions, TurnContext turnContext) {
            for (MessageReaction reaction : messageReactions) {
                ...
                return log.find(turnContext.getActivity().getReplyToId())
                        .thenCompose(resultActivity -> {
                        if (resultActivity == null)
                        {
                            ...
                            sendMessageAndLogActivityId(turnContext, String.format("Activity %s not found in the log.", turnContext.getActivity().getReplyToId()));
                        }
                        
                        sendMessageAndLogActivityId(turnContext, String.format("You added '%s' regarding '%s'", reaction.getType(), resultActivity.getText()));
                    }
                );
            }
